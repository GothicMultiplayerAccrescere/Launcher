...
void NatPunchthroughClient::OnPunchthroughFailure(void)
{
	...
	if (rakPeerInterface->GetConnectionState(sp.facilitator)!=IS_CONNECTED)
	{
		if (natPunchthroughDebugInterface)
		{
			...
			char guidString[128];
			sp.targetGuid.ToString(guidString, 128);
			natPunchthroughDebugInterface->OnClientMessage(SLNet::RakString("Not joined to facilitator, so cannot retry punchthrough after first failure. Returning failure onj guid %s, system address %s to user.", guidString, ipAddressString));
		}

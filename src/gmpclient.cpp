...
GMPClient::GMPClient(int serverId, QObject *pParent) :
    QObject(pParent),
    m_ServerId(serverId),
...
{
    connect(m_pSocket, &QUdpSocket::readyRead, [this]()
    {
        ...
        case 0x84:
        {
            ...
            else if(static_cast<uint8_t>(buf[1]) == 2)
            {
                ...
                qDebug() << "ServerVersion: " << version;
                emit serverChecked(m_ServerId, serverName, gamemode, version, player, bots, description);
            }
            break;
        }
        case 0xC0:
    ...
    connect(m_pSocket, &QUdpSocket::connected, [this]()
    {
